...
" -- migrated Plug 'scrooloose/nerdtree'        " File navigator
" -- migrated Plug 'bling/vim-airline'          " Vim fancy status line
" -- migrated Plug 'tpope/vim-surround'         " Surround, wrap or replace code with chars
" -- migrated Plug 'tpope/vim-fugitive'         " Git in vim
" INCOMPATIBLE WITH DELIMATE Plug 'tpope/vim-endwise'          " Complete blocks do/end
" -- migrated Plug 'tpope/vim-commentary'       " Toggle comments
" -- migrated Plug 'tpope/vim-dispatch'         " Asynchronous build and test dispatcher
" -- discared Plug 'ervandew/supertab'          " Perform all your vim insert mode completions with Tab
" -- replaced Plug 'NLKNguyen/papercolor-theme' " Best colorscheme
" -- migrated Plug 'diepm/vim-rest-console'     " REST console
" -- migrated Plug 'vim-test/vim-test'          " Run test file from vim
" -- discarded Plug 'wincent/ferret'             " Enhanced multi-file search for Vim
" -- discarded Plug 'junegunn/goyo.vim'          " Focus mode for writing
" -- replaced Plug 'Raimondi/delimitMate'       " Auto close special chars {}, [], ()
" -- migrated Plug 'alvan/vim-closetag'         " Auto close xml, html tags
" -- migrated Plug 'thinca/vim-localrc'         " Local vimrc config
" -- replaced Plug 'w0rp/ale'                   " Linter & Async engine for code analysis
" -- discarded Plug 'mattn/emmet-vim'            " Expansions
" -- migrated Plug 'AndrewRadev/switch.vim'     " Switch / Toggle words

" -- replaced Plug 'neoclide/coc.nvim', {'branch': 'release'}

" let g:coc_disable_startup_warning = 1

" -- replaced Plug 'sheerun/vim-polyglot'       " Syntax support

" -- replaced Plug 'SirVer/ultisnips'           " Ultimate solution for snippets
" -- migrated Plug 'ap/vim-css-color'           " Colorize hexadecimal colors
" -- migrated Plug 'wellle/targets.vim'         " Modify faster (){}[] contents
Plug 'AndrewRadev/splitjoin.vim'  " Better Split/Join lines
" -- migrated Plug 'rhysd/vim-grammarous'       " Spell and grammar checks
Plug 'mogelbrod/vim-jsonpath'     " Navigating JSON document
" -- migrated Plug 'wellle/targets.vim'         " Add more text objects
" -- discarded Plug 'rodjek/vim-puppet'          " Puppet syntax support

" replaced Plug 'maxmellon/vim-jsx-pretty'
" Plug 'leafgarland/typescript-vim' " Syntax & Typescript support
" Plug 'peitalin/vim-jsx-typescript' "Syntax & Typescript support

" replaced Plug 'junegunn/fzf', { 'dir': '~/.fzf', 'do': './install --all' } " Fuzzy file finder
" replaced Plug 'junegunn/fzf.vim'
...
" -- migrated autocmd BufWritePre * %s/\s\+$//e " Remove trailing spaces

" let test#strategy = "dispatch"
" let test#ruby#rspec#executable = 'SKIP_PULL=1 /srv/app/bin/container-run bundle exec spring rspec %'

" migrated let mapleader=" "   " Sets the leader key

" let g:move_key_modifier = 'C' " Move plugin

" let delimitMate_matchpairs = "(:),[:],{:}" " Autoclose just some chars

" migrated let g:closetag_filenames = '*.html,*.xhtml,*.phtml' " These are the file extensions where this plugin is enabled.
...
" Sequences
" discarded nmap <leader>r :for i in range(1,10) \| put ='192.168.0.'.i \| endfor

" " Goyo mode toggler
" discarded map <leader>z :Goyo<CR>
